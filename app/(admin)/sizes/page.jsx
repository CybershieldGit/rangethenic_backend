...
export default function SizesPage() {
  ...
  const loadAttributes = async () => {
    try {
      setLoading(true);
      const data = await fetchAttributes("size");
      setAttributes(data);
    } catch (err) {
      console.error(err);
      showNotification(err.message || "Failed to load sizes", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async (e) => {
    ...
    setAdding(true);
    try {
      const created = await createAttributeAPI("size", val);
      setAttributes((prev) => [...prev, created].sort((a, b) => a.value.localeCompare(b.value)));
      setNewValue("");
      setIsModalOpen(false);
      showNotification(`Size "${val}" successfully created.`);
    } catch (err) {
      showNotification(err.message || "Failed to add size", "error");
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleDelete = async (id, val) => {
    ...
    try {
      ...
      showNotification(`Size "${val}" removed.`);
    } catch (err) {
      showNotification(err.message || "Failed to delete size", "error");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-[60vh] gap-6">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[#b89b5e] animate-pulse">Loading Sizes...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12 md:mb-16 px-4 sm:px-0">
        <div>
          <span className="text-[#b89b5e] font-black tracking-[0.5em] uppercase text-[9px] md:text-[10px] block mb-2 md:mb-3">— Size Settings —</span>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-[#2b2622] leading-none mb-2">Size Filter</h1>
          <p className="text-[#6f6a65] text-xs max-w-md leading-relaxed opacity-60 italic">Define valid dimension sizes for your products.</p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 bg-[#fcfbf9] z-10">
                <tr className="border-b border-[#f2eee9]">
                  <th className="p-6 font-black text-[#6f6a65]/40 text-[9px] uppercase tracking-[0.3em]">Available Sizes</th>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#2b2622]/40 backdrop-blur-md z-[200] flex items-center justify-center p-4 transition-all">
          <div className="bg-white w-full max-w-md rounded-[32px] border border-[#e8e1d9] p-8 shadow-[0_30px_70px_rgba(0,0,0,0.15)] relative animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-black uppercase tracking-widest text-[#2b2622] mb-6">Add Size</h3>
            <form onSubmit={handleAdd} className="space-y-6">
              <div>
                <label className="block text-[9px] uppercase font-black tracking-widest text-[#6f6a65] mb-2.5 ml-1">Size Value</label>
                <input 
                  type="text" 
                  value={newValue} 
                  onChange={(e) => setNewValue(e.target.value)} 
                  required 
                  placeholder="e.g. XS, M, Free Size" 
              ...
              >
                {adding ? <LoadingSpinner /> : "Add Size"}
              </button>
            </form>
          </div>
        </div>
