...
import { createProduct, uploadImage, uploadVideo, fetchCategories, createCategory as createCategoryAPI, deleteCategory as deleteCategoryAPI } from "@/utils/api";
...
export default function CreateProductPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    category: "", countInStock: 0, isBestSeller: false,
    isCODAllowed: true,
  });
              ...
              {showNewCategory ? (
                <div className="flex gap-2">
                  ...
                  <button type="button" disabled={creatingCategory || !newCategoryName.trim()}
                    onClick={async () => {
                      ...
                      try {
                        const created = await createCategoryAPI(newCategoryName.trim());
                  ...
                  >
                    {creatingCategory ? <LoadingSpinner size="w-3 h-3" /> : 'Add'}
                  </button>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1" ref={categoryRef}>
                    ...
                    {categoryDropdownOpen && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#e8e1d9] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] z-50 overflow-hidden max-h-56 overflow-y-auto">
                        {categories.length === 0 ? (
                          <div className="p-4 text-center text-[#6f6a65]/40 text-xs italic">No categories yet. Click + New to create one.</div>
                        ) : (
                          categories.map((cat, idx) => (
                            <button key={cat._id || cat.id || idx} type="button"
                              onClick={() => {
                  ...
                  >
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest hidden sm:inline">New</span>
                  </button>
                  {formData.category && (
                    confirmDeleteCat ? (
                      <div className="flex items-center gap-1.5">
                        <button type="button" disabled={deletingCategory}
                          onClick={async () => {
                            ...
                            try {
                              ...
                              if (catToDelete) {
                                ...
                                setCategories(prev => prev.filter(c => c._id !== catToDelete._id));
                                setFormData(prev => ({ ...prev, category: "" }));
                                showNotification(`Category "${catToDelete.name}" deleted.`);
                              }
                            } catch (err) {
                              showNotification(err.message, "error");
                            } finally {
                        ...
                        >
                          {deletingCategory ? <LoadingSpinner size="w-3 h-3" /> : 'Yes'}
                        </button>
                        <button type="button" onClick={() => setConfirmDeleteCat(false)}
                      ...
                      <button type="button" onClick={() => setConfirmDeleteCat(true)}
