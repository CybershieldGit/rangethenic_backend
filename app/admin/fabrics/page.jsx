  ...
  return (
    <div className="max-w-4xl mx-auto relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12 md:mb-16 px-4 sm:px-0">
        <div>
          <span className="text-[#b89b5e] font-black tracking-[0.5em] uppercase text-[9px] md:text-[10px] block mb-2 md:mb-3">— Weave Chamber —</span>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-[#2b2622] leading-none mb-2">Fabric Filter</h1>
          <p className="text-[#6f6a65] text-xs max-w-md leading-relaxed opacity-60 italic">Define fabric materials for your offerings.</p>
        </div>
        <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#fcfbf9] border-b border-[#f2eee9]">
                  <th className="p-6 font-black text-[#6f6a65]/40 text-[9px] uppercase tracking-[0.3em]">Manifested Fabrics</th>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#2b2622]/40 backdrop-blur-md z-[200] flex items-center justify-center p-4 transition-all">
          <div className="bg-white w-full max-w-md rounded-[32px] border border-[#e8e1d9] p-8 shadow-[0_30px_70px_rgba(0,0,0,0.15)] relative animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-black uppercase tracking-widest text-[#2b2622] mb-6">Manifest Fabric</h3>
            <form onSubmit={handleAdd} className="space-y-6">
              ...
              >
                {adding ? <LoadingSpinner /> : "Manifest Fabric"}
              </button>
            </form>
          </div>
        </div>
